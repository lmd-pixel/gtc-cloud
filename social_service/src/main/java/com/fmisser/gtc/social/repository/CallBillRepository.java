...
public interface CallBillRepository extends JpaRepository<CallBill, Long> {
    ...
    @Query(value = "SELECT tc.duration AS duration, tc.created_time AS createTime, " +
            ...
            "LEFT JOIN t_call_bill tcb ON tcb.user_id_to = tc.user_id_to AND tcb.type = tc.type AND valid = 1 " +
    ...
    @Query(value = "SELECT tc.duration AS duration, tc.created_time AS createTime, " +
            ...
            "LEFT JOIN t_call_bill tcb ON tcb.user_id_to = tc.user_id_from AND tcb.type = tc.type AND valid = 1 " +
